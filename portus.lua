		...
		lua_thread.create(function()
			while not isCharInAnyCar(PLAYER_PED) do
				if timer < os.clock() then
					break
				elseif getDistanceBetweenCoords3d(pos.fX, pos.fY, pos.fZ, getCharCoordinates(PLAYER_PED)) <= 0.8 then
					--coord(x, y, z)
...
getForwardPosAngle = function()
	...
	return ffi.new("vector3", { mX + math.sin(-angle) * 2.0, mY + math.cos(-angle) * 2.0, mZ - 1.0 }), angle
end

addParticleWithoutRand = function(particle, position, velocity, particleData)
...
createPortal = function(pos, angle)
	...
	for i = 0.0, 3.1, 0.1 do
		addParticleWithoutRand(fxC.prt_sand2, ffi.new("vector3", { pos.fX - sin_ * 1.0, pos.fY - cos_ * 1.0, pos.fZ + i }), velocity, fxPrtFrame)
		addParticleWithoutRand(fxC.prt_sand2, ffi.new("vector3", { pos.fX + sin_ * 1.0, pos.fY + cos_ * 1.0, pos.fZ + i }), velocity, fxPrtFrame)
	end
	
	for i = 0.0, 1.1, 0.1 do
		addParticleWithoutRand(fxC.prt_sand2, ffi.new("vector3", { pos.fX - sin_ * i, pos.fY - cos_ * i, pos.fZ + 3.0 }), velocity, fxPrtFrame)
		addParticleWithoutRand(fxC.prt_sand2, ffi.new("vector3", { pos.fX + sin_ * i, pos.fY + cos_ * i, pos.fZ + 3.0 }), velocity, fxPrtFrame)
	end
